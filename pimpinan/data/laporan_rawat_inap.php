    ...
    var dataSets = [{
        ...
        data: [<?php
                for ($i = 2015; $i <= $tahun; $i++) {
                    $query_bulan = mysqli_query($conn, "SELECT COUNT(kode_rawat_inap) as jumlah FROM rawat_inap WHERE YEAR(tgl_masuk_rawat) = $i");
        ...
        data: [<?php
                for ($i = 2015; $i <= $tahun; $i++) {
                    $query_bulan = mysqli_query($conn, "SELECT COUNT(kode_rawat_inap) as jumlah FROM rawat_inap WHERE YEAR(tgl_keluar_rawat) = $i");
                    $pen_line = mysqli_fetch_assoc($query_bulan);
                    if ($pen_line == null) {
                        echo 0;
                    } else {
